...
// notepad++ search example (WIG).*?(_txt_db)
...
if ( $error == 1 ){WIG_toastr("$result" , "my_pos=tr" , "delay=1100");}
if ( $error == 1 ){echo "<script>alert('$result');</script>";} 
}


// HELP log to a global log and console
...
$my_date=date("d/m/Y : H:i:s", time());
echo "<script type=\"text/javascript\">console.log( \"$my_date : $txt\");</script>";
$txt=$my_date . ": " . $txt . "\n";
...
WIAG_bs($GLOBALS["WAR_logger"]);$my_filename=$GLOBALS["WAR_logger"]["filename"];
...
// echo "<br> $old_size , $new_size , $num_bytes";  
   ...
   {if ( strlen("$value") >= 3 ){echo "<br>$key => $value";}}
   fclose($file);
}
...
{
$GLOBALS["WAR_progres"]=array_merge($GLOBALS["WAR_progress"],func_get_args());
